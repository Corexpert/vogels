...
Table.prototype.get = function (hashKey, rangeKey, options, callback) {
  ...
  if (self.schema._mapAlias) {
    if(_.isPlainObject(hashKey)){
      hashKey = self.schema.serializeItem(hashKey);
    }

    if (options.AttributesToGet) {
      var toGet = [];
      _.each(options.AttributesToGet, function(a) {
        toGet.push(self.schema._mapName[a].value);
      });
      options.AttributesToGet = toGet;
    }
    if (options.ProjectionExpression) {
      options.ProjectionExpression = self.serializeProjectionExpression(options.ProjectionExpression.split(','));
    }
    if (rangeKey) {
      rangeKey = self.schema.serializeItem(rangeKey);
    }
  }
  ...
  self.docClient.getItem(params, function (err, data) {
    ...
    if(self.schema._mapAlias){
      data.Item = self.schema.deserializeItem(data.Item);
    }
    if(data.Item) {
...
Table.prototype.create = function (item, options, callback) {
  ...
  var start = function (callback) {
    if(self.schema._mapAlias){
      item = self.schema.serializeItem(item);
    }
  ...
  internals.callBeforeHooks(self, 'create', start, function (err, data) {
    ...
    self.docClient.putItem(params, function (err) {
      ...
      if(self.schema._mapAlias){
        item.attrs = self.schema.deserializeItem(item.attrs);
      }
  ...
  internals.callBeforeHooks(self, 'update', start, function (err, data) {
    ...
    self.docClient.updateItem(params, function (err, data) {
      ...
      if(data.Attributes) {
        ...
        if(self.schema._mapAlias){
          result.attrs = self.schema.deserializeItem(result.attrs);
        }
      }
...
Table.prototype.destroy = function (hashKey, rangeKey, options, callback) {
  ...
  options = options || {};
   ...
   if (self.schema._mapAlias && _.isPlainObject(hashKey)) {
      hashKey = self.schema.serializeItem(hashKey);
  }
  ...
  self.docClient.deleteItem(params, function (err, data) {
    ...
    if(data.Attributes) {
      if(self.schema._mapAlias){
        data.Attributes = self.schema.deserializeItem(data.Attributes);
      }
...
internals.deserializeItems = function (table, callback) {
  ...
  return function (err, data) {
    ...
    if(table.schema._mapAlias){
      result.Items = _.map(result.Items, function(i) {
        i.attrs = table.schema.deserializeItem(i.attrs);
          return i;
      });
    }
...
Table.prototype.runBatchGetItems = function(params, callback) {
  var self = this;
  if (self.schema._mapAlias) {
    _.each(params.RequestItems, function(t, k) {
      var opts = {};
      _.each(t, function(options, k) {
        if (k === 'AttributesToGet') {
          if (!opts[k]) {
            opts[k] = {};
          }
          var toGet = [];
      ...
      _.merge(params.RequestItems[k], opts);
    });
  }
  
  self.docClient.batchGetItem(params, function(err,data){
    if(err){
      return callback(err);
    }
    else{
      if(self.schema._mapAlias){
        _.each(data.Responses,function(t,k) {
          var rows = [];
          _.each(t,function(r) {
            rows.push(self.schema.deserializeItem(r));
          });
          data.Responses[k] = rows;
        });
      }
      callback(err,data);
    }
...
Table.prototype.runBatchWriteItem = function(params, callback) {

  var self = this;
  self.docClient.batchWriteItem(params, function(err,data){
    if(err){
      return callback(err);
    }
    else{
...
Table.prototype.serializeProjectionExpression = function(projection, map) {
  var toProject = [];
  map = map || this.schema._mapName;
  _.each(projection, function(p) {
    if (p.indexOf('.') !== -1) {
      var proj = p.split('.');
      var tempMap = map;
      var concatString = [];
      
      _.each(proj, function(p) {
        p = p.trim();
        concatString.push(tempMap[p].value);
        tempMap = tempMap[p].content;
      });
      
      toProject.push(concatString.join('.'));
    }
    else {
      toProject.push(map[p.trim()].value);
    }
  });
  return toProject.join(', ');
};