...
internals.configSchema = Joi.object().keys({
  ...
  autoRename : Joi.boolean().default(true)
}).required();
...
var Schema = module.exports = function (config) {
  ...
  Joi.validate(config, internals.configSchema, { context: context }, function (err, data) {
    ...
    self.autoRename = data.autoRename;
    ...
    if(self.timestamps) {
      ...
      if(self._mapAlias){
        _.merge(self._mapAlias, {createdAt:{ value : 'createdAt'}});
        _.merge(self._mapAlias, {updatedAt:{ value : 'updatedAt'}});
        _.merge(self._mapName, {createdAt:{ value : 'createdAt'}});
        _.merge(self._mapName, {updatedAt:{ value : 'updatedAt'}});
      }
