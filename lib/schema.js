...
var Schema = module.exports = function (config) {
  ...
  if(config.useAlias){
    this._mapAlias = this.createMapName(config.schema);
    this._mapName = this.createMapAlias(config.schema);
    config.schema = this.convertToName(config.schema);
    config.hashKey = this._mapName[config.hashKey];
    if(config.rangeKey)
    {
      config.rangeKey = this._mapName[config.rangeKey];
    }
    if(config.indexes) {
...
Schema.prototype.createMapName = function(config) {
  ...
  _.each(config, function(c, k) {
    if (_.isPlainObject(c)) {
      if (!c.type) {
        obj[c.name] = self.createMapName(c);
      }
      else {
        obj[c.name] = k;
      }
