...
describe('Vogels Integration alias Tests', function() {
  ...
  before(function(done) {
    ...
    User = vogels.define('vogels-int-test-alias-user', {
      hashKey: 'id',
      useAlias: true,
      schema: {
        id: {
          name: 'i',
          type: Joi.string().required().default(uuid.v4)
        },
        email: {
          name: 'e',
        ...
        age: {
          name: 'a',
          type: Joi.number().min(10)
        },
        roles: {
          name: 'r',
          type: vogels.types.stringSet().default(['user'])
        },
        acceptedTerms: {
          name: 'at',
    ...
    Tweet = vogels.define('vogels-int-test-alias-tweet', {
      hashKey: 'UserId',
      rangeKey: 'TweetID',
      useAlias: true,
      schema: {
        ...
        PublishedDateTime: {
          name: 'pdt',
          type: Joi.date().default(Date.now)
        }
      },
      indexes: [{
        hashKey: 'UserId',
    ...
    Movie = vogels.define('vogels-int-test-alias-movie', {
      hashKey: 'title',
      timestamps: true,
      useAlias: true,
      schema: {
